...
module.exports.controller = function(router,passport) {
	...
	.post(function(req,res,next){
	  	passport.authenticate('local',function(err,user,info){
 			...
 			if(!user){
 				resData.error = true;
 				resData.data = {};
 				resData.userMessage = "Invalid email or password.";
	...
	router.route('/profile').get(session.checkToken,methods.profile);
	router.route('/user/pay').put(session.checkToken,methods.pay);
	router.route('/user/network')
	.post(session.checkToken,methods.addNetwork)
	.delete(session.checkToken,methods.removeNetwork)
	.get(session.checkToken,methods.userNetwork);
...
methods.updateReminder = function(req,res) {
	res.send(200);
};
